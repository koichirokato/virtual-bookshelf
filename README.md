...
- 📝 **個人メモ**: 本ごとのレビューとおすすめ文
- 🔍 **検索・フィルター**: タイトル・著者・星評価・レベル・言語での絞り込み
- 📊 **読書統計**: 総蔵書数の表示
...
- 🏷️ **技術書タグ**: レベル・言語・ジャンルのタグをブラウザから編集可能
...
4. **フィルター**: サイドバーで星評価・レベル・言語で絞り込み
5. **詳細表示**: 本をクリックして詳細モーダルを表示
6. **星評価**: モーダル内で1-5星の評価を設定・変更・リセット

### 技術書タグの編集
本の詳細モーダルで「✏️ 編集」をクリックすると、以下のフィールドを設定できます。

| フィールド | 内容 | 選択肢 |
|-----------|------|--------|
| レベル | 難易度 | 入門 / 中級 / 上級 |
| 言語 | 対象プログラミング言語（複数可） | C++ / Python / Rust / JavaScript / Go / 言語非依存 |
| ジャンル | 技術分野（自由入力） | ロボティクス / AI開発 / アルゴリズム など |

設定したタグはカードに表示され、サイドバーのフィルターで絞り込めます。
Kindleインポートや手動追加時は空欄のまま追加され、後から編集できます。
